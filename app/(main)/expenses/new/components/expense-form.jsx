...
import { useCreateExpenseMutation } from "@/redux/services";
...
export function ExpenseForm({ type = "individual", onSuccess }) {
  ...
  const onSubmit = async (data) => {
    try {
     ...
     splits: formattedSplits
      ...
      reset(); // Reset form
     //console.log("participants in form submit:",participants)
  ...
  // console.log("participants :",participants,"paidBy :",paidByUserId,"split chck:",splits)
                ...
                if (!selectedGroup || selectedGroup.id !== group.id) {
                  ...
                  if (group.members && Array.isArray(group.members)) {
                    const members = group.members.map( mem => {
                      const {_id,...memDeatils} = mem;
                      return  {id:_id,...memDeatils}
                    }       
                    )
        ...
        <div className="space-y-2">
          ...
          >
            ...
            {participants.map((participant,index) => (
              <option key={participant.id||index} value={participant.id}>
